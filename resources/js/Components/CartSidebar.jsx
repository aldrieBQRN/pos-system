            ...
            <div className="flex flex-col h-full relative bg-white">
                ...
                <div className="px-4 pt-4 mb-2 flex justify-between items-center border-b pb-3 bg-white">
                    ...
                    <div className="flex gap-2">
                        {cart.length > 0 && (
                            <>
                                <button onClick={handleHoldOrder} className="p-2 text-yellow-500 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition-colors border border-transparent hover:border-yellow-200" title="Hold Order">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M14.25 9v6m-4.5 0V9M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                </button>
                                <button onClick={handleClearCart} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-200" title="Clear Cart">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                                </button>
                            </>
                        )}
                        <button onClick={toggleSenior} className={`p-2 rounded-lg transition-colors border ${isSenior ? 'bg-yellow-100 text-yellow-700 border-yellow-300' : 'bg-white text-gray-400 border-gray-200 hover:bg-gray-50'}`} title="Toggle PWD/Senior Discount">
                            ...
                            <div key={item.id} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm flex flex-col gap-2">
                                ...
                                <div className="flex justify-between items-center mt-1">
                                    ...
                                    <div className="flex items-center bg-gray-50 rounded-lg border border-gray-200 h-8">
                                        <button onClick={() => removeFromCart(item.id)} className="w-8 h-full flex items-center justify-center text-gray-500 hover:bg-gray-200 hover:text-red-500 rounded-l-lg transition-colors">
                ...
                <div className="border-t p-5 bg-gray-50">
                    {isSenior && (
                        <div className="space-y-1 mb-3">
                            ...
                            <div className="flex justify-between text-sm text-green-600 font-medium"><span>Less: Senior/PWD (20% + VAT)</span><span>-{formatPrice(discount)}</span></div>
                        </div>
                    )}
                    ...
                    <button onClick={() => setShowPaymentModal(true)} disabled={cart.length === 0} className="w-full py-3.5 rounded-xl bg-green-600 text-white font-bold text-lg shadow-lg hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-transform active:scale-95 flex justify-center gap-2 items-center">
                        <span>Checkout</span>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden text-center p-6 flex flex-col items-center">
                        ...
                        <div className="w-full space-y-3">
                            ...
                            <button
                                ...
                                className="w-full py-3.5 border border-gray-300 text-gray-700 font-bold rounded-xl hover:bg-gray-50 active:scale-95 transition-all"
                            >
                                New Order
                            </button>
                        </div>
