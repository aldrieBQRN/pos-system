        ...
        const startScanning = async () => {
            try {
                ...
                await html5QrCodeRef.current.start(
                    activeCameraId,
                    config,
                    async (decodedText) => { // Made async
                        const currentTime = Date.now();
                        if (currentTime - lastScanTime.current < 2000) return; // 2s Cooldown

                        // --- FIX: Wait for Parent to Confirm Success ---
                        const result = await onScan(decodedText);

                        // Only show success if onScan returns TRUE
                        if (result === true) {
                            ...
                            // If result is false (e.g. Out of Stock), prevent immediate rescan
                            lastScanTime.current = currentTime;
                        }
                    },
                    (err) => {}
            ...
            <div className="bg-white w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl relative flex flex-col">
                <div className="p-4 bg-gray-50 border-b flex justify-between items-center shrink-0">
                    ...
                    <div className="flex gap-2">
                        {cameras.length > 1 && (
                            <button onClick={handleSwitchCamera} className="w-8 h-8 flex items-center justify-center bg-blue-100 rounded-full text-blue-600 hover:bg-blue-200 transition-colors">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
                                </svg>
                            </button>
                        )}
                        <button onClick={onClose} className="w-8 h-8 flex items-center justify-center bg-gray-200 rounded-full text-gray-600 hover:bg-red-100 hover:text-red-500 font-bold transition-colors">&times;</button>
                    </div>
                </div>

                <div className="relative flex-1 bg-black min-h-[300px] flex items-center justify-center overflow-hidden">
                    <div id="reader" className="w-full h-full" style={{ transform: isMirrored ? 'scaleX(-1)' : 'none' }}></div>
                    {errorMessage && <div className="absolute inset-0 flex items-center justify-center p-6 bg-gray-900 text-white text-center z-20"><div><p className="font-bold mb-1">Camera Error</p><p className="text-sm text-gray-300">{errorMessage}</p></div></div>}
                ...
                <div className={`p-4 text-center text-sm font-medium transition-colors duration-300 shrink-0 ${isCooldown ? 'bg-green-100 text-green-800' : 'bg-white text-gray-500'}`}>
